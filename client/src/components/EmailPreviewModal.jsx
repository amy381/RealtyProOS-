...
export default function EmailPreviewModal({ task, tx, commissions = {}, collaborators = {}, tcSettings = [], driveFolderId = null, onUpdate, onClose }) {
  ...
  const ccEmails = ccResult.emails.length > 0
    ...
    : typeof template?.cc === 'string' && template.cc
      ? resolveVars(template.cc, tx, tcSettings, commissions, collaborators).split(',').map(s => s.trim()).filter(Boolean)
      : []

  const resolvedSubject = resolveVars(template?.subject || '', tx, tcSettings, commissions, collaborators)
  const resolvedBody    = resolveVars(template?.body    || '', tx, tcSettings, commissions, collaborators)
